...
As an example, `launch 4.23,https://github.com/openshift/cluster-kube-scheduler-operator/pull/123 aws` would launch an OpenShift 4.23 cluster with the changes made in that PR running on AWS.
...
The test binary and suite definitions live in [`cmd/cluster-kube-scheduler-operator-tests-ext`](cmd/cluster-kube-scheduler-operator-tests-ext/main.go).
...
```bash
# Run a suite
./cluster-kube-scheduler-operator-tests-ext run-suite openshift/cluster-kube-scheduler-operator/operator/serial
...
./cluster-kube-scheduler-operator-tests-ext run-suite openshift/cluster-kube-scheduler-operator/operator/serial --junit-path=/tmp/junit.xml
```
